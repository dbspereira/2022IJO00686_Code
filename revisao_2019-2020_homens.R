...
# SÓ HOMENS
NH_G1 <- NH_G1 %>% filter(RIAGENDR==1)
NH_G2 <- NH_G2 %>% filter(RIAGENDR==1)
NH_G3 <- NH_G3 %>% filter(RIAGENDR==1)
NH_G4 <- NH_G4 %>% filter(RIAGENDR==1)
...
# SÓ HOMENS
NH_EN1 <- NH_EN1 %>% filter(RIAGENDR==1)
NH_EN2 <- NH_EN2 %>% filter(RIAGENDR==1)
NH_EN3 <- NH_EN3 %>% filter(RIAGENDR==1)
NH_EN4 <- NH_EN4 %>% filter(RIAGENDR==1)
...
ggplot(NH_G, aes(x=idadedoimc, y=media_imc, group=geracao, colour=geracao)) + 
  ...
  labs(colour = "Generation", x = 'age (years)', y='Body Mass Index (Kg/m²)',
       tag = 'Fixed-effects Multilevel Analysis by Generation Stratification - Males',
...
ggplot(NH_EN, aes(x=idadedoimc, y=media_imc, group=estado_nutricional, color=estado_nutricional)) + 
  ...
  labs(colour = "Income",x = '', y='Body Mass Index (Kg/m²)',
       tag = 'Fixed-effects Multilevel Analysis by Nutritional Status at 25 years old Stratification - Males',
       caption = "Source: NHANES 1999-2020 Adults (18 to 65 years old)") +
  theme(plot.title.position = "plot",
        plot.title = element_text(face="bold"),
        plot.caption = element_text(hjust=0, face = "italic", color = "darkgray"),
