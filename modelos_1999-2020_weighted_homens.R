...
# Gerações
NH_G1 <- read.csv('NHS_molted_G1_FINALM.csv')
NH_G1 <- merge(NH_G1, PESO, by = "id")

NH_G2 <- read.csv('NHS_molted_G2_FINALM.csv')
NH_G2 <- merge(NH_G2, PESO, by = "id")

NH_G3 <- read.csv('NHS_molted_G3_FINALM.csv')
NH_G3 <- merge(NH_G3, PESO, by = "id")

NH_G4 <- read.csv('NHS_molted_G4_FINALM.csv')
NH_G4 <- merge(NH_G4, PESO, by = "id")

# Estado Nutricional
NH_EN1 <- read.csv('NHS_molted_EN1_FINALM.csv')
NH_EN1 <- merge(NH_EN1, PESO, by = "id")

NH_EN2 <- read.csv('NHS_molted_EN2_FINALM.csv')
NH_EN2 <- merge(NH_EN2, PESO, by = "id")

NH_EN3 <- read.csv('NHS_molted_EN3_FINALM.csv')
NH_EN3 <- merge(NH_EN3, PESO, by = "id")

NH_EN4 <- read.csv('NHS_molted_EN4_FINALM.csv')
...
modelo_final_hlm3 <- lme(fixed = imc ~ idadedoimc,
                         random = ~ 1| id,
                         data = NH_G1,
                         method = "REML")
summary(modelo_final_hlm3)
tab_model(modelo_final_hlm3)
...
MSE_G1
LogLikelyhood_G1 <- logLik(mod_G1, REML = F)
LogLikelyhood_G1
...
idadedoimc_G1 <- 40:65
G1 <- rep("1950 or earlier",25)

mod_GI_use <- summary(mod_G1)
media_imc_G1 <- mod_GI_use$coefficients[,1]
media_imc_G1 # 23.277216 intercept #24.1299 2
media_imc_G1 <- media_imc_G1 + 24.1299 
media_imc_G1[1] <- media_imc_G1[1] - 24.1299 
...
media_imc_G2 # 23.73 intercept #24.30
media_imc_G2 <- media_imc_G2 + 24.30
media_imc_G2[1] <- media_imc_G2[1] - 24.30
...
media_imc_G3 # 23.13 intercept #23.42
media_imc_G3 <- media_imc_G3 + 23.42
media_imc_G3[1] <- media_imc_G3[1] - 23.42
...
media_imc_G4 # 24.33 intercept #24.97
media_imc_G4 <- media_imc_G4 + 24.97
media_imc_G4[1] <- media_imc_G4[1] - 24.97
media_imc_G4
...
media_imc_EN1 # 16.5993022  intercept #16.10
media_imc_EN1 <- media_imc_EN1 + 16.10 
media_imc_EN1[1] <- media_imc_EN1[1] - 16.10 
...
media_imc_EN2 # 20.8470499 #20.74
media_imc_EN2 <- media_imc_EN2 + 20.74
media_imc_EN2[1] <- media_imc_EN2[1] - 20.74
media_imc_EN2
...
media_imc_EN3 # 25.5184096 intercept #25.05
media_imc_EN3 <- media_imc_EN3 + 25.05
media_imc_EN3[1] <- media_imc_EN3[1] - 25.05
media_imc_EN3
...
media_imc_EN4 # 33.95421 intercept #33.25
media_imc_EN4 <- media_imc_EN4 + 33.25 
media_imc_EN4[1] <- media_imc_EN4[1] - 33.25 
media_imc_EN4
